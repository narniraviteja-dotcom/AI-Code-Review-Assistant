...
function History() {
  ...
  const fetchReviews = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:5000/api/reviews/history?page=${page}&limit=5`);
      setReviews(res.data.reviews || []);
      setPagination(res.data.pagination || { page: 1, totalPages: 1, total: 0, hasMore: false });
    } catch (error) {
